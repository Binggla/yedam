 ...
 * SEND(문자전송) 예제입니다.
 */
public class SendExample extends BlogDAO {
	public static void sendPwSms(String toPhone, String userId)  {
		/*
		...
		set.put("to", toPhone); // 수신번호

		// 10월 16일 이후로 발신번호 사전등록제로 인해 등록된 발신번호로만 문자를 보내실 수 있습니다.
		set.put("from", "01099619775"); // 발신번호
		set.put("text", user.getUserId() + "의 비밀번호는 " + user.getUserPw() + "입니다."); // 문자내용
...
//		set.put("mode", "test"); // test모드 수신번호를 반드시 01000000000 으로 테스트하세요. 예약필드 datetime는 무시됨. 결과값은 60. 잔액에서 실제
//									// 차감되며 다음날 새벽에 재충전됨
