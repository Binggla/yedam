...
// filter() : 조건이 참인 요소를 새로운 배열로 반환.
let newAry = objAry.filter(function(val){
...
function femaleList (ary) {
  ...
  ary.forEach(function(val) {
    ...
    for (let field in val) {
      tag += `${field} : ${val[field]}${field === 'ip_address'?'':', '}`;
    }
    tag += '</li>'
  });
  tag += '</ul>'
...
//     let val = '';
//     for (let field in val) {
//       val += `${field} : ${val[field]} ${idx===(ary.length-1)?'':','}`;
//     }
//     li.textContent = val;
//     ul.appendChild(li);
//   });
//   target.appendChile(ul);
// }
